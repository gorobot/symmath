...
public:
  ...
  explicit inline LessThan(const T1 &lhs, const T2 &rhs);

private:

  template< typename U >
  friend inline void
  assign_(U &lhs, const LessThan<T1, T2> &rhs) {
  ...
  template< typename U >
  friend inline void
  assign_add_(U &lhs, const LessThan<T1, T2> &rhs) {
  ...
  template< typename U >
  friend inline void
  assign_sub_(U &lhs, const LessThan<T1, T2> &rhs) {
...
template< typename T1,
          typename T2 >
inline LessThan<T1, T2>::LessThan(const T1 &lhs, const T2 &rhs)
  : lhs_(lhs),
    rhs_(rhs) {}
...
#endif // SYMMATH_OPERATIONS_INEQUALITY_LESS_THAN_HPP
