...
public:
  ...
  explicit inline ExpectedValue(const T1 &lhs, const T2 &rhs);

private:

  template< typename U >
  friend inline void
  assign_(U &lhs, const ExpectedValue<T1, T2> &rhs) {
  ...
  template< typename U >
  friend inline void
  assign_add_(U &lhs, const ExpectedValue<T1, T2> &rhs) {
  ...
  template< typename U >
  friend inline void
  assign_sub_(U &lhs, const ExpectedValue<T1, T2> &rhs) {
...
template< typename T1,
          typename T2 >
inline ExpectedValue<T1, T2>::ExpectedValue(const T1 &lhs, const T2 &rhs)
  : lhs_(lhs),
    rhs_(rhs) {}
