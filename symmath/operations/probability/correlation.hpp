#ifndef SYMMATH_OPERATIONS_PROBABILITY_CORRELATION_HPP
#define SYMMATH_OPERATIONS_PROBABILITY_CORRELATION_HPP
...
public:
  ...
  explicit inline Correlation(const T1 &lhs, const T2 &rhs);

private:

  template< typename U >
  friend inline void
  assign_(U &lhs, const Correlation<T1, T2> &rhs) {
  ...
  template< typename U >
  friend inline void
  assign_add_(U &lhs, const Correlation<T1, T2> &rhs) {
  ...
  template< typename U >
  friend inline void
  assign_sub_(U &lhs, const Correlation<T1, T2> &rhs) {
...
template< typename T1,
          typename T2 >
inline Correlation<T1, T2>::Correlation(const T1 &lhs, const T2 &rhs)
  : lhs_(lhs),
    rhs_(rhs) {}
