...
class Vector
  : public has_value_array<T, Length>,
    public has_addition<Vector<T, Length>>,
    public has_additive_inverse<Vector<T, Length>>,
    public has_assignment<Vector<T, Length>>,
    public has_multiplication<Vector<T, Length>>,
    public has_subtraction<Vector<T, Length>>,
    private Tensor {
public:
...
template< typename U >
inline void
Vector<T, Length>::assign_mul(const U &rhs) {
  auto result = result_of<U>::value(rhs);
  for(size_t i = 0; i < Length; i++) {
    this->value_[i] *= result[i];
  }
